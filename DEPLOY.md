...
- **Обзор** — плитки (обращений, доля «нет в базе», доля ошибок, ожидание
  p50/p95, языки, печать бланков), график обращений по суткам с выделенной
  долей без ответа, задержки по стадиям STT/RAG/TTS. Только этот раздел уходит
  на печать: это и есть справка руководству.
