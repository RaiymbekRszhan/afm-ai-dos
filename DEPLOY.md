...
Страница `http://СЕРВЕР/admin?token=…` — таблица всех точек из `deploy/kiosks.txt`:
на связи или молчит, сколько вопросов было, кнопка «Отключить/Включить» и общий
рубильник «тех. работы» на все точки разом. Пишет в тот же
`kiosks-disabled.txt`, что правят по ssh, — два пути управления одним состоянием.
