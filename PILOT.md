...
### Запасной путь: вариант 2 «видео-аватар» (без Unreal)

Если render-ПК недоступен, не приехал на площадку или Unreal подводит — есть
**вариант 2** на `:8100` ([video_ui/README.md](video_ui/README.md)): тот же голосовой
пайплайн, но вместо реалтайм-рендера крутятся записанные ролики (idle + «говорит»).
Липсинка нет, зато нужен **только Linux-сервер** — ни Windows, ни GPU-рендера, нагрузка
почти нулевая. Поднимается второй командой рядом с основным стеком:

```bash
bash run_api.sh        # основной бэкенд (:8000)
bash video_ui/run.sh   # вариант 2 (:8100), в другом терминале
```

Оба варианта работают одновременно — можно показать рядом и сравнить. Вариант 2 не
трогает вариант 1: вопросы он проксирует на `:8000`.
