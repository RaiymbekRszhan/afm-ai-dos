...
_NORM_COMMON = [
    (r"https?://", ""),
    # «Insider» транслит-фолбэк читал как «инсидер» (i -> и): в названии канала
    # «АФМ Insider» это слышно сразу, поэтому слово — в словарь (отзыв 17.08).
    (r"\bInsider\b", "инсайдер"),
...
_NORM_KK_SIMPLE = [
    ...
    # ⚠️ МРП/АЕК: в казахском ответе аббревиатура оставалась нераскрытой (она была
    # только в русской таблице), и модель её молча ПРОПУСКАЛА — «жеке тұлғаға
    # қырық …» без единицы измерения, штраф превращался в бессмыслицу (киоск
    # 17.08). АЕК — казахское написание того же показателя.
    (r"\b(?:МРП|АЕК)\b", "айлық есептік көрсеткіш"),
    (r"№\s*", "нөмір "),
]
