...
class Settings(BaseSettings):
    ...
    #
    # sentence_cap — до какой длины предложение НЕ РЕЖЕМ. Ставился 600 («не рвать
    # юридическую фразу посреди»), но замер 17.08 это опроверг: OmniVoice
    # СЛУЧАЙНО выбрасывает куски текста, и вероятность растёт с длиной запроса.
    # Один и тот же текст, 8 прогонов, пропуск ловился распознаванием:
    #     165 симв. -> 0/8      233 симв. -> 0/8
    #     298 симв. -> 1/8      354 симв. -> 4/8
    # Поэтому здесь ВЫБРАНА ПРАВИЛЬНОСТЬ, а не гладкость: выброшенный пункт из
    # ответа про ответственность хуже слышимого шва на запятой (на шве пауза
    # половинная, 150 мс, — звучит как вдох). 230 — верхняя длина, на которой
    # пропусков не было ни разу. Поднимать ТОЛЬКО с новым замером.
    tts_kk_sentence_cap: int = 230
