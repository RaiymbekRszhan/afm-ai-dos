...
# Слово/фраза, повторённые подряд 3+ раза, — признак «залипания» распознавания
# (Whisper-подобные движки на шуме и обрывках уходят в петлю). В логах киоска
# 29.07: «Елбасы, елбасы, елбасы…» полсотни раз, и такой «вопрос» уезжал в RAG
# целиком — поиск шёл по мусору, а на экране была простыня вместо вопроса.
...
def collapse_repeats(text: str) -> str:
    ...
    «Елбасы, елбасы, елбасы…» -> «Елбасы». Работает по словам, а не по строке:
    между повторами у движка обычно стоят запятые и пробелы. Ищем повторы фраз
    длиной 1-3 слова — так ловится и «деду, деду, деду», и «тихо тихо тихо».
    Регистр и знаки при сравнении игнорируем, в тексте сохраняем ПЕРВОЕ
    вхождение как есть (оно, как правило, единственное осмысленное).
    """
    if not text or not text.strip():
        return text
    tokens = re.findall(r"\S+", text)
    key = [re.sub(r"[^\w]+", "", t.lower()) for t in tokens]
    out: list[str] = []
    i = 0
    while i < len(tokens):
        collapsed = False
        # От КОРОТКОЙ единицы к длинной: «елбасы×30» должно схлопнуться в одно
        # слово, а не в тройку (сначала пробовали наоборот — тест поймал).
        # «а б а б а б» на size=1 не срабатывает (соседи разные) и корректно
        # ловится на size=2.
        for size in (1, 2, 3):
            if i + size * _REPEAT_MIN > len(tokens):
                continue
            block = key[i:i + size]
            if not any(block):          # блок из одних знаков — не повтор
                continue
            reps = 1
            j = i + size
            while j + size <= len(tokens) and key[j:j + size] == block:
                reps += 1
                j += size
            if reps >= _REPEAT_MIN:
...
def looks_degenerate(text: str) -> bool:
    """Похоже ли распознанное на залипание движка, а не на вопрос гражданина.

    Длинная реплика, в которой почти нет уникальных слов, — это петля, а не
    вопрос: отвечать на неё нечем, и RAG всё равно вернёт что-то случайное
    (в логах: на «Елбасы×50» пришёл общий текст про Агентство). Короткие
    реплики не трогаем: «да, да, да» — нормальное согласие.
    """
    words = re.findall(r"[^\W\d_]+", (text or "").lower())
    if len(words) < 8:
        return False
    return len(set(words)) / len(words) < _DEGENERATE_RATIO


def not_recognized_phrase(language: str | None = None) -> str:
    """Что сказать, когда распознать вопрос не удалось (петля/шум)."""
    if _is_kk(language):
        return ("Сұрағыңызды анық естімедім. Микрофонға жақынырақ тұрып, "
                "қайта сұрап көріңізші.")
    return ("Я не расслышал вопрос. Подойдите ближе к микрофону и "
            "спросите ещё раз, пожалуйста.")
