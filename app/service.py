...
# Фразы, которые движки STT выдают НА ТИШИНЕ — куски субтитров из обучающих
# данных. Все три встретились на киоске 29.07 («Продолжение следует.»,
# «Редактор субтитров А.Семкин Корректор А.Кулакова»). Сравниваем по вхождению
# в нормализованный текст: движок иногда добавляет к ним точку или имя.
_STT_ARTIFACTS = (
    "продолжение следует",
    "редактор субтитров",
    "корректор",
...
# Одна и та же буква подряд 4+ раз («Пссссссс») — не слово, а шум, который
# движок попытался расшифровать.
_CHAR_RUN = re.compile(r"([^\W\d_])\1{3,}", re.UNICODE)


def looks_not_speech(text: str) -> bool:
    """Похоже ли распознанное на шум, а не на вопрос гражданина.

    Три признака, все с живого киоска:
      * известная фраза-артефакт («Продолжение следует.» на пустой записи);
      * буквенная «каша» вроде «Псссссссс» — растянутый шум;
      * слишком короткий обрывок («Пи»), который вопросом быть не может.
    Согласие («да», «иә») коротким обрывком НЕ считаем: это рабочая реплика в
    диалоге уточнения. Ошибиться здесь дорого в обе стороны, поэтому пороги
    выбраны консервативно — лучше пропустить кривой вопрос (RAG вернёт отказ и
    предложит уточнение), чем отшить настоящий.
    """
    raw = (text or "").strip()
    if not raw:
        return True
    low = _norm_for_compare(raw)
    if any(a in low for a in _STT_ARTIFACTS):
        return True
    if is_affirmative(raw):
        return False
    if _CHAR_RUN.search(raw):
        return True
    letters = re.findall(r"[^\W\d_]", raw)
    # 4 буквы — это «Пи», «Ага» без гласной и подобный мусор. Настоящий вопрос
    # короче не бывает даже одним словом («штраф» — 5 букв).
...
def looks_degenerate(text: str) -> bool:
    ...
    if len(words) >= 8 and len(set(words)) / len(words) < _DEGENERATE_RATIO:
        return True
    # Короткая реплика, которая ЦЕЛИКОМ состоит из повторов: «Тихо, тихо, тихо»
    # (тоже с киоска). Длинной петлёй она не считается — слов мало, — но и
    # вопросом не является. Согласие («да, да, да») исключаем отдельно.
